...
class PosPackagedCard(models.Model):
    ...
    # Stored image copied from product (variant) or template, reliable in Kanban
    ...
    def _compute_card_image(self):
        for rec in self:
            ...
            prod = rec.product_id
            if prod:
                img = getattr(prod, 'image_128', False) or getattr(prod.product_tmpl_id, 'image_128', False)
            ...
            if prod:
                try:
                    if getattr(prod, "image_128", False):
                        url = f"/web/image?model=product.product&id={prod.id}&field=image_128"
                    else:
                        pt = getattr(prod, "product_tmpl_id", False)
                        if pt and getattr(pt, "image_128", False):
                            url = f"/web/image?model=product.template&id={pt.id}&field=image_128"
                except Exception:
                    # never break kanban if images aren't accessible
                    url = ""
            rec.image_url = url or "/web/static/img/placeholder.png"
...
@api.model
def action_mark_all_confirmed_done(self):
    self.search([('state', '=', 'confirmed')]).write({'state': 'done'})
    return {
        "type": "ir.actions.act_window",
        "name": _("POS Packaged Delivery Board"),
        "res_model": "pos.packaged.card",
