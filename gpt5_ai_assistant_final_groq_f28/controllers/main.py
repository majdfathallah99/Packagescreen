...
<script>(function(){{
  ...
  if (micBtn && SR){{
    try{{
      ...
      micBtn.addEventListener('click', function(){{ try{{ setStatus('Listening…'); rec.start(); }}catch(e){{ setStatus(''); }} }});
  ...
  window.startLive = function(){{
    ...
    if (live) return; live = true; setStatus('Listening…');
    try{{
      recog = new SR(); recog.lang='ar-SA'; recog.continuous=true; recog.interimResults=true;
      let partial = '';
      recog.onresult = function(ev){{ try{{ for(let i=ev.resultIndex;i<ev.results.length;i++){{ var r=ev.results[i]; partial += r[0].transcript; if(r.isFinal){{ if(input) input.value = partial.trim(); autoSend(); partial=''; }} else {{ if(input) input.value = partial; }} }} }}catch(e){{}} }};
      recog.onerror = function(e){{ setStatus('Mic error'); }};
      recog.onend = function(){{ if(live){{ try{{ recog.start(); }}catch(e){{}} }} else {{ setStatus(''); }} }};
      recog.start();
    }}catch(e){{ setStatus('Mic blocked'); }}
  }};
  window.stopLive = function(){{ live = false; try{{ recog && recog.stop(); }}catch(e){{}} setStatus(''); }};
...
function speak(text){{ if(!window.speechSynthesis) return; const u = new SpeechSynthesisUtterance(text); u.lang = (/[\u0600-\u06FF]/.test(text) ? 'ar-SA' : 'en-US'); window.speechSynthesis.speak(u); }}
</script>
</body></html>"""
