...
(function(){{
  ...
  function loadVoicesOnce(){{
    return new Promise((resolve)=>{{
      try{{
        ...
        // Fallback: resolve after 1s even if empty
  ...
  async function pickVoice(lang2){{
    ...
    if(!v){{
      const target = langMap[lang2] || 'en-US';
  ...
  // Auto-speak last assistant reply (only once per page)
  try{{
    ...
    if(txt){{
      ...
      if(prev!==hash){{
        const uPromise = speak(txt);
        sessionStorage.setItem(key, hash);
        Promise.resolve(uPromise).then(u=>{{
          if(u){{ u.onstart = function(){{ try{{ window.__liveActive=false; }}catch(e){{}} }}; u.onend = function(){{ try{{ window.__liveActive=true; window.startLive(true); }}catch(e){{}} setTTS(''); }}; }}
        }});
      }}
    }}
  }}catch(e){{}}
  ...
  // ---- Live STT with dynamic language ----
  let recog = null;
