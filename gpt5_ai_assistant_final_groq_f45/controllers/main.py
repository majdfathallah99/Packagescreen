...
def _env(danger=False):
    ...
    Some hosted builds may not expose env.sudo(); fall back to with_user(1) if available."""
    env = request.env
    if danger:
        try:
            # Standard Odoo
            ...
            # Fallback for stacks without .sudo()
            try:
...
def _persist_history(history):
    try:
        ...
        # Mark session as modified if available (some stacks require this to save nested changes)
...
def _ai_reply(user_text):
    ...
    # Server-side shortcut first (no model call)
    ...
    if getattr(msg, "tool_calls", None):
        ...
        # Cap history to last 1000 messages
        if len(history) > 1000:
    ...
    def chat(self, **post):
        ...
        for _m in reversed(history):
            if (_m or {}).get('role') == 'assistant':
                last_assistant = (_m or {}).get('content') or ''
                break
...
class AIAssistantLiveController(http.Controller):
    @http.route(['/ai_assistant/api/send'], type='http', auth='user', methods=['POST'], csrf=False)
    def api_send(self, **post):
        # Accept JSON or form; return plain JSON (no JSON-RPC wrapper)
        try:
            ...
            try:
                ...
                data = _json.loads((request.httprequest.data or b'').decode('utf-8') or '{}')
            except Exception:
                data = {}
        msg = (data.get('message') or '').strip()
        if not msg:
            body = json.dumps({'ok': False, 'error': 'empty'})
            return request.make_response(body, headers=[('Content-Type','application/json')])
        try:
            reply = _ai_reply(msg)
            if not isinstance(reply, str):
                reply = str(reply or '')
            body = json.dumps({'ok': True, 'reply': reply})
        except Exception as e:
...
(function(){
  ...
  function normalize(s){ return (s||'').toLowerCase().replace(/[\p{P}\p{S}]/gu,' ').replace(/\s+/g,' ').trim(); }
  function similar(a,b){
    a = new Set(normalize(a).split(' ')); b = new Set(normalize(b).split(' '));
  ...
  async function sendAjax(text){
    ...
    try{
      const r = await fetch('/ai_assistant/api/send', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({message:text})});
      if(!r.ok){ const t = await r.text(); appendMessage('Assistant', '⚠ '+r.status+' '+t); return; }
      let data=null; try{ data = await r.json(); }catch(e){ const t = await r.text(); appendMessage('Assistant', '⚠ bad JSON '+t); return; }
      if(data && (data.reply || (data.result && data.result.reply))){ const reply = data.reply || data.result.reply; lastAssistant = reply;
  ...
  function startLive(){
    ...
    recog.onend = function(){ setStatus('idle'); if(liveActive && !ttsPlaying && now()>ttsCooldownUntil){ try{recog.start();}catch(e){} } };
