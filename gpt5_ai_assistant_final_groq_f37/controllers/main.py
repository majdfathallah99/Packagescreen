...
<script>(function(){{
  ...
  // Live AJAX sender with dynamic append and TTS
  ...
  async function sendAjax(text){
    try{
      ...
      if(data && data.reply){
        ...
        if(u){ u.onend = function(){ try{ if(window.__liveActive) startLive(true); }catch(e){} }; }
  ...
  function autoSend(text){
    try{
      if (!text){ var t = (input && input.value)||''; text = t.trim(); }
      if(!text) return;
