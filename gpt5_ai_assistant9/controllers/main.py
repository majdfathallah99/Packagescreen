...
class AiAssistantController(http.Controller):
    ...
    def chat(self, prompt=None, history=None):
        ...
        system_prompt = (icp.get_param("ai_assistant.system_prompt", default="You are a helpful AI assistant inside Odoo.") or "").strip()

        if not api_key:
            return {"ok": False, "error": "API key is not set. Open GPT-5 Assistant Settings to configure it."}

        if OpenAI is None:
            return {"ok": False, "error": "Python package 'openai' not installed on the server."}
    ...
    def app_page(self, **kw):
        # Render inline page template
        return request.render("gpt5_ai_assistant.page_chat_inline", {})
