...
class PriceCheckerWizard(models.TransientModel):
    ...
    def _onchange_autoscan(self):
        for wiz in self:
            if not wiz.barcode:
                continue
            product = self.env['product.product'].search([('barcode', '=', wiz.barcode)], limit=1)
            if product:
                wiz.product_id = product.id
                wiz.product_name = product.display_name
                wiz.uom_name = product.uom_id.name
                wiz.list_price = product.lst_price
                wiz.price = product.lst_price


    def _compute_price(self, product):
        # Simplified: use product list price
