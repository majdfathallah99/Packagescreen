...
<script>(function(){
  ...
  function populateVoices(){
    ...
    voices.forEach((v,i)=>{
      ...
      if (firstArabicIndex === -1 && (l.startsWith('ar') || n.includes('arab'))) firstArabicIndex = i;
    });
    // Default to Arabic voice if available
    if (firstArabicIndex >= 0) {
      ttsSel.value = 'idx:'+firstArabicIndex;
    }
  ...
  function pickVoiceAutoByText(text){
    try{
      const voices = speechSynthesis.getVoices()||[];
      // Prefer Arabic by default
      let best = null;
      for(const v of voices){
        const l=(v.lang||'').toLowerCase(), n=(v.name||'').toLowerCase();
        if(l.startsWith('ar') || n.includes('arab')){ best=v; break; }
      }
      if (best) return best;
      const want = isArabicText(text||'') ? 'ar' : (detectLang(text||'')||'en').slice(0,2);
      for(const v of voices){
        const l=(v.lang||'').toLowerCase(), n=(v.name||'').toLowerCase();
        if(l.startsWith(want)){ return v; }
      }
      return voices[0] || null;
    }catch(e){ return null; }
  }

  window.__liveActive = False = false;
  ...
  async function speak(text){
    ...
    // Prefer selected voice; default selection is Arabic if available
    let v = null;
    ...
    else { u.lang = isArabicText(text||'') ? 'ar-SA' : 'ar-SA'; } // default Arabic
  ...
  if (micBtn && SR){
    try{
      const rec = new SR();
      rec.lang = 'ar-SA'; rec.continuous=false; rec.interimResults=true; // default Arabic
      let finalText='', interim='';
      micBtn.addEventListener('click', function(){
        if (!window.isSecureContext) { alert('Voice input requires HTTPS.'); return; }
        try{
          setStatus('Listening…'); finalText=''; interim='';
          rec.lang = (langSel?.value==='ar' ? 'ar-SA' :
                      langSel?.value==='en' ? 'en-US' : 'ar-SA'); // default Arabic
          rec.start();
        }catch(e){ setStatus(''); }
      });
  ...
  function startLive(resume){
    ...
    rec.onresult = function(ev){
      ...
      // If auto mode and Arabic detected, switch to Arabic
  ...
  if ('speechSynthesis' in window){
    ...
    if (ttsTest){
      ttsTest.addEventListener('click', async function(){
        ...
        const sample = 'اختبار الصوت يعمل الآن.';
        await speak(sample);
      });
    }
  }else{
    ...
    def chat(self, **post):
        ...
        <form id="ai_form" method="post" action="/ai_assistant">
            ...
            <div style="margin-top:8px;font-size:90%%">
              ...
              <select id="sr_lang">
                <option value="ar" selected>Arabic (ar-SA)</option>
