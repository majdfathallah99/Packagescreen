...
class PriceCheckerWizard(models.TransientModel):
    ...
    def _compute_other_uoms_html(self, product):
        ...
        pkg_section = ""
        p_lines = []
        Packaging = self.env['product.packaging'].with_context(active_test=False)
        # Some databases don't have product_tmpl_id on product.packaging.
        # Search by all variant product_ids of this template to be safe.
        variant_ids = product.product_tmpl_id.product_variant_ids.ids if product.product_tmpl_id else [product.id]
        pkgs = Packaging.search([('product_id', 'in', variant_ids)])
        for p in pkgs:
