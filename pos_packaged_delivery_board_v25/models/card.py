...
class PosPackagedCard(models.Model):
    ...
    def _sync_window_start(self):
        # Use context override or default 90 days
        days = self.env.context.get("ppdb_days", DEFAULT_WINDOW_DAYS)
        return fields.Datetime.now() - timedelta(days=days)

    @api.model
    def create_from_pos_lines(self, date_from=False, states=("paid","done","invoiced")):
        ...
        for line in lines:
            ...
            if not used_uom or used_uom.id == default_uom.id:
                continue
            vals = {
            ...
            try:
                ...
                # uniqueness violations or concurrent loads are fine
                pass
        return created

    @api.model
    def action_refresh_board(self):
        # Manual refresh from the Action menu: last 90 days
        self.create_from_pos_lines(self._sync_window_start(), ("paid","done","invoiced"))
        return {
    ...
    def cron_auto_sync(self):
        # Cron: keep board updated without any popup
        self.create_from_pos_lines(self._sync_window_start(), ("paid","done","invoiced"))
        return True
