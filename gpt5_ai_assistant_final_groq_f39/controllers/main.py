...
class AIAssistantController(http.Controller):
    ...
    def api_send(self, **post):
        msg = (post.get('message') or '').strip() if isinstance(post, dict) else ''
        if not msg:
            return {'error': 'empty'}
        try:
            reply = _ai_reply(msg)
    ...
    def live_chat(self, **kw):
        # Standalone live voice chat page (safe, no dependencies)
        html = '''<!doctype html>
...
(function(){
  ...
  function startLive(skipWake){
    ...
    recog.onerror = function(e){ setStatus('error'); };
  ...
  liveBtn.addEventListener('click', function(){ startLive(); });
