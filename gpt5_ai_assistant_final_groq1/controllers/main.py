...
def _normalize_base_url(url):
    if not url:
        return "https://api.groq.com/openai/v1"  # Groq default
    u = (url or "").strip()
    if not u:
...
def _model_name():
    return (_get_conf("ai_assistant.model") or "llama-3.3-70b-versatile").strip()
    ...
    def settings(self, **post):
        ...
        if request.httprequest.method == 'POST':
            ...
            if 'base_url' in post:
                ...
                msg += "Saved Base URL."
            if 'model' in post:
                _set_conf("ai_assistant.model", post.get('model') or 'llama-3.3-70b-versatile')
                if msg: msg += " "
                msg += "Saved Model."
        ...
        model = _get_conf("ai_assistant.model") or "llama-3.3-70b-versatile"
